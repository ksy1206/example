...
<script type="text/javascript">
	...
	function update_notice(){
		...
		var notice_no = $('#notice_no_hidden').val();
		var title = $('#title').val();
		var content = $('#content').val();
		...
		var params = "notice_no="+notice_no+"&title="+title+"&content="+content+"&file_01_name="+file_01_name+"&file_02_name="+file_02_name+"&file_03_name="+file_03_name+"&file_01_path="+file_01_path+"&file_02_path="+file_02_path+"&file_03_path="+file_03_path;
		
		
        $.ajax({
            type        : "POST" 
          , async       : true
          , url         : "/admin/update_notice.do"
          , data        : params
          , dataType    : "json"
          , timeout     : 30000  
          , cache       : false    
...
<div class="container">
  <h2>공지사항등록</h2>
  <form>
  	<input type="hidden" id="notice_no_hidden" name="notice_no_hidden" value="${list[0].notice_no}">
    ...
    <div class="form-group">
       <label for="content">내용:</label>
 	   <textarea class="form-control" rows="5" id="content">${list[0].content}</textarea>
    </div>
  </form>
  ...
  <div style="text-align: center;">
  	<button type="button" class="btn btn-primary"  onclick="chk_update();">수정</button>
  </div>
